...
- 2026-06-25: AI Search rearchitected from "send whole DB to one generative call" → embeddings-based semantic retrieval. ROOT CAUSE of the chronic "No matches found / try rephrasing": the generative call (`gemini-2.5-flash`, a thinking model) was failing/empty almost every time on this free-tier key — confirmed `gemini-2.5-flash` and `gemini-2.0-flash` both return 429/`limit:0` for generateContent; only `gemini-2.5-flash-lite` has working generate quota. New pipeline in `server/services/gemini.js`: (1) embed every club/community/student once via `gemini-embedding-001` (`text-embedding-004` is 404 on this key; no sync `batchEmbedContents`, so per-item `embedContent` at concurrency 8, `outputDimensionality:768`, cached in-memory keyed by `type:id`+content-hash, re-embedded only on text change); (2) per query embed the query (`RETRIEVAL_QUERY` vs `RETRIEVAL_DOCUMENT` task types) and rank locally by cosine; (3) `gemini-2.5-flash-lite` writes one-line reasons AND judges `relevant` (precision filter on top of embedding recall), best-effort with 1 retry + 6s timeout, synthesized reasons on failure.
- 2026-06-25: Embedding cosine scores from `gemini-embedding-001` are COMPRESSED (related + unrelated both ~0.5–0.7), so absolute thresholds don't work. Selection is RELATIVE: require top score ≥ `MIN_TOP_SCORE` (0.62) else treat as no-match, then keep the cluster within `SCORE_GAP` (0.07) of the top. These two consts are tunable and were calibrated against live embeddings (blockchain→WEB3+Blockchain Forums; webrtc→Web Dev Forum+full-stack student; music→keyword fallback→Music Club; quantum→empty).
- 2026-06-25: `getModel()` default changed `gemini-2.5-flash` → `gemini-2.5-flash-lite` (the only model with free generate quota) — this also unblocks admin/club AI Insights which were silently 429-ing. Reliability decoupled: embeddings (high free limits, no thinking, ~always succeed) own "did we find matches"; the generative call only decorates with reasons and degrades gracefully. Keyword search retained as last-resort fallback when even the embedding API errors. No new env vars (uses existing `GEMINI_API_KEY`); supersedes the 2026-05-15 `gemini-2.5-flash` search decisions above.

- 2026-06-25: AI Search upgraded to a TWO-STAGE HYBRID (embeddings recall → flash-lite reasoning) to restore the depth the pure-embedding version lost. Stage 1: embeddings gate on `MIN_TOP_SCORE` (0.60) then pull a generous shortlist (within `SHORTLIST_BAND` 0.15 of top, capped `SHORTLIST_SIZE` 15) — wide band intentionally lets broad/multi-focus entities (e.g. GDSC, whose description only mentions "web" in passing) reach the reasoner instead of being cut by a tight cluster. Stage 2: `gemini-2.5-flash-lite` reads the shortlist's full text, judges `relevant` (precision filter) and writes 1-2 sentence reasons citing specific skills/tags/phrases (lifted the old 20-word cap; maxOutputTokens 2048→3072). Verified live: "web development" now returns GDSC + Web Dev Forum + students with detailed reasons; "blockchain"→WEB3+Blockchain Forums; "quantum computing"→empty. Cost: warm queries ~7-8s (was 1-3s) from larger payload + richer output; no-match stays ~0.5s. Replaces the tight-cluster `SCORE_GAP` selection from earlier today.
