...
import { X, Loader, Image, Calendar, MapPin } from 'lucide-react';
...
const CreatePostModal = ({ isOpen, onClose, type, onCreated }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setTitle(''); setBody(''); setImage(''); setEventDate('');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`${API}/api/posts`, {
        type, title, body, image, eventDate: eventDate || undefined, venue, tag,
  ...
  const accentColor = type === 'Recruitment' ? 'amber' : 'blue';
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-[#18181b] border border-white/10 w-full max-w-lg rounded-2xl p-6 relative shadow-2xl overflow-hidden"
            >
